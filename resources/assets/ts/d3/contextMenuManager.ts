...
const contextMenuManager = function (isTouchDevice) {
    // floating
    let bgCtxMenu: any, nodeCtxMenu: any, groupCtxMenu: any;
    const ctxMenuMaxSize = {width: 370, height: 305};
    // fixed
    let bgMenu: any, nodeMenu: any, groupMenu: any;
    ...
    function createBgCtxMenu() {
        const menuHtml = '<ul class="dropdown-menu" role="menu">'
    ...
    function updateRemoveFromGroupsItem(groupsList, callback) {
        ...
        if (groupsList && groupsList.length) {
            ...
            groupsList.forEach(function (g:any) {
                let text = g.text.split(/\r\n|\r|\n/),
    ...
    function _addEventsToNode(d3selection: any, d3container: any, cfg: any) {
        if (!d3selection || d3selection.empty())
            return;
        ...
        d3selection.on(isTouchDevice ? 'touchstart' : 'contextmenu', function (node: any) {
            ...
            if (
                ...
                && viewport.width()
                && (
                    viewport.width() < ctxMenuMaxSize.width
                    || viewport.height() < ctxMenuMaxSize.height
                )
            ) {
                hide();
    ...
    function enable($element: any) {
        if (!$element)
            return;
        $element.removeClass('disabled');
    }

    function disable(element: any) {
        if (!element)
            return;
    ...
    function addEvents(type: string, object: any, container: any, cfg: any) {
        hide();
        if (type == 'group')
    ...
    function getContextMenu(type: any) {
        if (type == 'group')
            return groupCtxMenu;
