import d3 from "d3";
import {util} from "./familyTreeUtil";
import actionHistory from "./actionHistory";
import tooltipManager from "./tooltipManager";
import contextMenuManager from "./contextMenuManager";

const familyTree = function (isTouchDevice){
    var dataNodes = [];
    var dataRelLinks = [];
    var dataChildLinks = [];
    var dataGroups = [];
    ...
    const HistoryManager = actionHistory();
    const TooltipManager = tooltipManager(isTouchDevice);
    const CtxMenuManager = contextMenuManager(isTouchDevice);

    function getSelectionCount(){
        let count = dataNodes.filter(function(n){return n.selected;}).length;
    ...
    function _detectIfTouchingOverNode(touch){
        ...
        if (target && target.classList && target.classList.contains('node')){
            ...
            if (isCreatingChild && tmpChildLink && !d3Node.classed('child-node')){
                ...
                var rel = getRelLinkById(relId)
                let isParent = rel.fromNode == node.id || rel.toNode == node.id;
                d3Node.classed('child-node', !alreadyExist && !isParent);
            }
    ...
    function _renderRelLink(d3selection, link, xs=0, ys=0, scale=1){
    ...
    function moveObjects(objects, deltaX, deltaY) {
        if (isNaN(deltaX) || isNaN(deltaY)) {
            console.error('moving nodes to NaN', objects);
            return;
        }
    ...
    var init = function(dom, w, h){
        ...
        const submitEvent = (isTouchDevice) ? 'touchstart' : 'click';
        ...
        if (gpopup){
            ...
            gpopup.on(submitEvent, '#submit', function(){
                const form = gpopup.find('.form-horizontal');
                let groupId = form.find('#input-id').val();
                if (!form.valid())
                    return;
                var records = form.serializeArray(),
                    values = {};
    ...
    var load = function(jsonData:any=null){
